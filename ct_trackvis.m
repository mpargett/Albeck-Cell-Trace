%CT_TRACKVIS
%   A plotting routine for visualizing cell track data.
%
%Usage:  
...
%       'stack' plots individual traces on closely stacked axes (the first
%           ntracks*nstacks traces, in order top to bottom - see below)
...
%   If using 'stack', the following options are available:
%   NTRACKS - a double indicating how many tracks to stack vertically
%   NSTACKS - a double indicating how many stacks to list horizontally
%   MORELABLES - set TRUE to label each track with Cell number
%   COLORFUL - set TRUE to plot tracks in different colors
...
%   For stack plotting
p.ntracks = 10;  p.nstacks = 3;  p.colorful = false;  p.morelabels = true;
...
if isempty(ah); nfh = figure; ah = axes;   else axes(ah);   end;
...
switch lower(type)
    ...
    case 'ensemble' % --Plots all tracks over time, overlayed--
        ...
        if size(cvals,1)==1; cvals(2,:) = cvals(1,:); end
    ...
    case 'histogram' % --Generates a time-lapse histogram (color-scaled)--
        ...
        %   Check if 0 is appropriate
        if ytl(1) > 0 && ytl(1) - ord <= 0; ad0 = 0; else ad0 = []; end  
        ytl = [ad0,ytl]; 
        %   Get tick positions for labels, and place labeling
        yt = nbins - cellfun(@(x)interp1([ad0,hx'],[ad0*0.5,1:nbins],x), ...
            num2cell(ytl)) + 1;
    ...
    case 'stack'
        ...
        ymx = max( max(max(d(1:p.nstacks*p.ntracks,:))), max(prctile(d,80)) );
        ymn = floor(ymn*10)/10;  ymx = ceil(ymx*10)/10;
        yrg = ymx - ymn;
        %   Set y-offsets
        pm = p.nstacks == 1;
        yoff = yrg*( p.ntracks-1 : -1 : 0 )' + ymx*pm;
        ytks = [yoff(1)+yrg;yoff];
        %Get x-scale (convert to real time?)
        xax = 1:nTime;  
        %Plot bounding box?
        %Set color 
        if p.colorful %Set cycling colors if desired
            pclr = p.cmap(round(size(p.cmap,1)*(0.2:0.2:0.8)),:);
        else
            pclr = interp1(cmap_x, p.cmap, 0.3);  mlow = pclr < max(pclr);
            pclr = pclr.*(mlow*0.5 + ~mlow*0.75);  %Darken and purify color
        end
        
        %Plotting (for each requested stack)
        for s = 1:p.nstacks
            if ~pm; sh = subplot(1, p.nstacks, s); else sh = ah; end
            %Subdivide data
            ti = (s-1)*p.ntracks + (1:p.ntracks);  dplt = d(ti,:);
            %Plot traces
            set(sh, 'ColorOrder', pclr); hold on; box on;
            plot(sh, (ones(p.ntracks,1)*xax)', ...
                (dplt -ymn + yoff*(ones(1,nTime)))',...
                'LineWidth', p.lw);
            %Draw sub-axis stubs and y-axis overlays
            line(sh, [0,ceil(nTime*0.05)], [ymn(pm);ytks(1:end-1)]*ones(1,2), ...
                'Color', 'k', 'LineWidth', 1);
            %Draw y-axis overlays
            line(sh, [xax(1),xax(1)], [ytks(1:end-1), ytks(2:end)], ...
                'LineWidth', 1);
            %Adjust axes and create x-axis label
            axis([xax(1), xax(end), ymn*pm-yrg/10, ...
                ymx*pm + yrg*p.ntracks + yrg/10]);
            set(sh, 'YTick', []);  xlabel('Time');
            
            if p.morelabels %Add Cell labels, if desired
                text(1.01*xax(end)*ones(p.ntracks,1) , yoff + yrg*0.5, ...
                    arrayfun(@(x)['C ',num2str(x)], ti, ...
                    'UniformOutput', false));
            end
        end
        
        %Plot mean/quantiles if desired
        if pm;  ct_trackvis(ah, 'mean', d, varargin{4:end});   
        else sh = subplot(1, p.nstacks, 1); end
        %Title and y-axis label
        set(sh, 'YTick', ymn*pm + [0, yrg], 'YTickLabels', [ymn, ymx]);
        ylabel('Intensity');   
        
        %Adjust figure size (if new figure was created)
        if exist('nfh','var'); set(nfh, 'Position', [20, 80, ...
            min(1600, p.nstacks*350), min(900, p.ntracks*100)]); end
